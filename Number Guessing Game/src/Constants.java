public class Constants {

    public static final String MESSAGE = "Enter your guess: ";
    public static final String DIFFICULTIES = "1. Easy - between 1 - 20\n" +
            "2. Medium - between 1 - 50\n" +
            "3. Hard - between 1 - 100\n";
    public static final String TITLE = "Number Guessing Game\n" +
             "Guess the number between 1 and X\n";
    public static final String TOO_HIGH = "\nToo high! Try again.";
    public static final String TOO_LOW = "\nToo low! Try again.";
    public static final String HIGH = "\nHigh! It's getting hot! Try again.";
    public static final String LOW = "\nLow! It's getting hot! Try again.";

}
